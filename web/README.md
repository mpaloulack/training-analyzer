...
A small web app that lets other people run the Training Analyzer script
...
`fetch_training_data.py`, and they download the resulting `training_data.json`.

**Live progress.** The backend streams the script's stdout to the browser as
newline-delimited JSON over the same POST, so users watch the same progress
they'd see in a terminal (`1/3 Activités…`, `100/130`, …). The closing JSON line
carries the output file (base64) and triggers the download — no server-side job
store, still fully stateless.
...
- **backend/** — FastAPI. One endpoint, `POST /api/run`, validates input, runs
  the analyzer script in an isolated temp dir, and streams back the JSON file.
- The analyzer script lives in the repo root and is copied into the backend
...
- **No persistence of secrets**: API key flows request → subprocess env →
  ...
  leak into logs or tracebacks. The output file is read into memory only for the response.
...
```bash
# Backend unit (validation, runner, packaging, API)
...
`MOCK_FETCH=1` makes the backend skip the real network fetch and emit a tiny
deterministic JSON file, so E2E/CI runs without Intervals.icu credentials.
